...
- What changed:
  - **Root cause of the operator's `ValueError: Invalid public key hex string: unknown` error:** `MeshCoreBackend._on_channel_message` fell back to the literal string `"unknown"` when the CHANNEL_MSG_RECV payload had no `pubkey_prefix` — which is **always**, because the MC protocol doesn't transmit a sender pubkey on channel broadcasts (by design: channel broadcasts are anonymous on a shared PSK). See `venv/lib/python3.12/site-packages/meshcore/reader.py:253-286` — CHANNEL_MSG_RECV payload has `channel_idx / path_len / txt_type / sender_timestamp / text` but no sender identity field. The "unknown" string flowed through as `backend_native_id`, producing a unified id of `mc:unknown` that got upserted into `contacts` on every channel message and surfaced in the UI as a DM-able contact. Every DM attempt to it blew up in `MeshCore.commands.send_msg(pubkey_hex, text)` because `"unknown"` isn't valid hex.
  - **Channel-message handler rewritten** ([radio/meshcore_backend.py](meshtastic-bridge/radio/meshcore_backend.py)). Now: (a) if the payload has a real hex `pubkey_prefix`, use it as before (future-proof for firmwares that do include one); (b) otherwise parse the common MC-companion-app convention of `"<Name>: <text>"` out of the message text to pull the sender's self-reported display name; (c) build a stable pseudo-id of shape `mc:anon:<channel>:<slug>` (slug = lowercased-alphanum of the parsed name, falling back to `"anon"`), so repeated messages from the same self-named sender aggregate into one "thread" without polluting the contact list with valid-hex IDs. `_on_contact_message` (DMs) now rejects any incoming with a non-hex pubkey_prefix instead of synthesizing a fake contact — DMs are guaranteed to carry pubkey per the MC protocol, so anything else is malformed.
  - **`send_direct_message` validates hex BEFORE handing off to the MC lib** ([meshcore_backend.py:108-130](meshtastic-bridge/radio/meshcore_backend.py)). Raises a plain-English `ValueError` ("channel-broadcast senders are anonymous and cannot receive DMs — reply on the public channel instead") instead of the lib's cryptic `"Invalid public key hex string: ..."`. New `_looks_like_hex()` helper used here + in both receive handlers.
  - **Dashboard server-side guard** ([dashboard.py:1114-1133](meshtastic-bridge/dashboard.py)). `/api/threads/<thread_id>/send` checks for `mc:unknown` or `mc:anon:*` thread ids at the top and returns a 400 with a user-readable error **before** any RadioManager path runs, plus marks the in-DB message row `delivery_status="failed"` with the same explanation.
  - **Dashboard float-window UX**. When the operator double-clicks an anonymous-channel-sender node on the canvas, the composer is now replaced with an explainer block — `◇ CHANNEL SENDER — CAN'T DM` + "MeshCore channel broadcasts don't carry a sender pubkey, reply on the PUBLIC CHANNEL · MESHCORE thread instead" — so nobody types a message, clicks SEND, and waits for a confusing error. The AI-reply toggle also disappears for anon nodes (no DM → no auto-reply). The TRACE button is hidden on all MC nodes (MC firmware doesn't expose traceroute; old UI showed TRACE, click fell through to a "not supported" toast).
  - **One-shot DB migration in `init_db()`** ([db/schema.py:151-173](meshtastic-bridge/db/schema.py)). On next bridge start, any existing legacy `mc:unknown` contact row + its message history are deleted. Clean slate for operators who've already been bitten by this bug. Migration logs a single `"removed N legacy mc:unknown contact row(s) and their messages"` line. No-ops on fresh installs.
- Why:
  - Operator report from live terminal: opened a DM thread on what looked like an MC peer, typed a message, got `TX via RadioManager failed: ValueError: Invalid public key hex string: unknown`. Three retries failed the same way. The AI auto-reply path had the same failure silently in the background. Root-caused to the receive-side `"unknown"` fallback in `_on_channel_message`. The fix closes the runtime path, but existing DB rows + the operator-visible UI still needed cleanup for a clean experience.
- Impact on project goals:
  - Unblocks cross-protocol messaging for operators running MC on public channels (the common real-world case). Anon channel senders now have a clearly non-DM-able UX; the operator sees the channel thread in the sidebar for replies.
  - Paves the way for the bridge's MC public-channel relay to work end-to-end: a peer on the MC mesh can broadcast → bridge sees it with a parsed display name → relay tag reads `from meshcore (<Name>): <text>` on the MT side → MT peers see a readable name instead of "mc:unknown."
  - Scope held: no changes to the `Relay.observe()` or policy paths. No new endpoints. Float-window DM rendering is layered on top of the existing thread/composer mechanism; legitimate MC contacts (real pubkey) keep their normal UI exactly as before.
- Files modified:
  - `meshtastic-bridge/radio/meshcore_backend.py` — new `_looks_like_hex()`; `_on_channel_message` rewritten (parse "Name: text", pseudo-id `mc:anon:<ch>:<slug>`); `_on_contact_message` validates hex + drops malformed; `send_direct_message` validates hex + raises plain-English error.
  - `meshtastic-bridge/dashboard.py` — `/api/threads/<id>/send` pre-routing guard for anon/unknown MC ids; `openFloatWindow` renders an explainer composer for anon nodes; `loadFloatData` omits AI toggle on anon nodes + TRACE button on all MC nodes.
  - `meshtastic-bridge/db/schema.py` — new `_cleanup_unknown_mc_contacts()` migration called from `init_db()`.
- Tests: 373/365 still green (no new failures; existing MC discovery tests cover the backend plumbing).

---

## [2026-04-20 17:30] — v2.6: Fix MT↔MC public-channel retransmission — dynamic MC channel resolution + MT ACK visibility

- What changed:
  - **MC channel resolution (the main fix).** `MeshCoreBackend` now enumerates the device's channel table at connect-time via a new `_discover_channels()` async method that calls `meshcore.commands.get_channel(idx)` for `idx in 0..15`, collecting `{idx, name, hash}` for every named slot, then picks the slot whose name reads as "public" (case-insensitive: matches `"public"`, `"Public Channel"`, `"public ch"`, `"#public"`). The resolved index is cached on `self._public_channel_index`. `send_broadcast(text, channel=0)` now substitutes that resolved index for the literal 0 before calling `send_chan_msg(resolved, text)`. Explicit non-zero channel callers pass through unchanged (preserves per-channel dashboard sends). Before: bridge-sourced MT→MC relays all went to raw slot 0 regardless of whether that was the operator's public channel — a very common misconfig because MC uses slot indices as opaque keys into a per-device channel table (unlike MT's unambiguous slot-0 = Primary convention). If slot 0 was a personal or unconfigured channel, every MT→MC message went into the void. After: MT→MC relays reach whichever slot the operator has named "Public."
  - **MT ACK visibility.** `_bridge_send` for Meshtastic switched from `wantAck=False` (fire-and-forget) to `wantAck=True, onResponse=self._on_bridge_ack_nak`. The callback name is literally `onAckNak` inside the MT interface's response-handler dispatch (see meshtastic-python `mesh_interface.py:1650-1654`) — that matters because the library routes plain ACKs (not just NAKs) to the handler only when named that way. Before: the bridge logged `relayed: +1` whether or not the radio actually put the packet on air, because `sendText` returns as soon as the packet is enqueued to the background serial/BLE thread. After: every bridge-sourced MT send produces a follow-up log line — `[bridge] MT ACK pkt_id=<N> from=<node>` on success or `[bridge] MT NAK pkt_id=<N> reason=<err>` on failure — visible in the Relay Health Live Log panel.
  - **Interface-ready gate.** New `_interface_ready()` helper in `standalone_bridge.py`: requires `self.interface` not None AND `_is_interface_alive()` AND `getattr(self.interface, 'myInfo', None) is not None`. `_bridge_send` now gates MT sends on this instead of the liveness-only check. Per meshtastic-python source (`mesh_interface.py:970-971`), `sendText` skips its internal `_waitConnected` barrier when `myInfo` is None — the packet enqueues into a half-initialized interface that silently drops it once the config handshake finishes. The new gate stops bridge sends until the handshake is actually complete.
  - **Relay Health panel surfaces the resolved MC slot.** `/api/bridge/health` gained `mc_public_channel_idx` (nullable int) and `mc_channels` (list of `{idx, name}`). The Block-1 "Backends connected" row now renders `mt ✓, mc ✓ · Public=ch<N>` when MC's Public slot is discovered, or a warning `⚠ no Public slot — using ch0 fallback (ch0:<name>, ch1:<name>)` when the device has channels but none named Public. Gives the operator instant visibility: if cross-protocol relay isn't reaching peers, the panel shows exactly which slot the bridge is using vs what's configured.
  - **New diagnostic log lines feeding the Live Log panel:**
    - `[mc] public channel resolved: idx=<N> name=<Name>` at connect-time, once
    - `[bridge] relay MC→MT: sendText pkt_id=<N> ch=<N> len=<N> wantAck=1` on every MT-destination send
    - `[bridge] relay MT→MC: send ch=0 (mc resolves ch0→<N>) len=<N>` on every MC-destination send
    - `[bridge] MT ACK pkt_id=<N> from=<node> — packet went on air` on ACK
    - `[bridge] MT NAK pkt_id=<N> reason=<err>` on NAK
    - All route through the existing `DashboardLogHandler` ring buffer and render colour-coded in the Relay Health Live Log.
- Why:
  - Operator observation on hardware: peer-to-bridge messages arrive cleanly (receive + observe + persistence all work); bridge logs `relayed: +1` for both directions. But remote peers on the other mesh never see the relayed message. Deep research across installed lib source (`venv/lib/python3.12/site-packages/{meshtastic,meshcore}/`) surfaced two root-cause candidates that precisely match "bridge thinks it sent but air doesn't see it":
    1. MC `send_chan_msg(idx, text)` takes a raw slot index, not a semantic channel identifier. MT's channelIndex=0 is unambiguous (first of 8 slots, by convention Primary/Public). MC's slot 0 could be anything the operator configured — most commonly a personal or unconfigured channel, NOT the "Public" channel they want to bridge. Hard-coding `channel=0` was the single biggest functional gap.
    2. MT `sendText(wantAck=False)` is fire-and-forget — the bridge had no signal of whether a packet actually transmitted. Combined with the `myInfo`-not-populated silent-drop behavior documented in mesh_interface.py:970, early sends after reconnect could enqueue against a half-initialized radio and vanish without trace.
- Impact on project goals:
  - v1.0-ship blocker for cross-protocol public-channel relay is resolved. The operator's stated goal — "message sent from a peer radio on public channel → bridge's radio receives it → bridge retransmits on the OTHER protocol's public channel → peers on that mesh see the relay" — now has all four steps instrumented and functional as long as the MC device has a channel named "Public."
  - Dashboard operator-observability milestone: every bridge-sourced send now produces a visible log line (pre-send + post-send) in the Live Log, including the MC slot remap decision and the MT ACK/NAK result. If a relay fails, the operator reads the reason string instead of staring at silent counters.
  - Scope held: no changes to `Relay.observe()`, the policy chain, dedup, rate-limiting, or the receive path. The v2.5 + v2.5.1 instrumentation surface stayed untouched; we added to it. `Policy.should_relay` tuple contract, `Relay.stats()` shape, the /api/bridge/health core fields — all backward-compat. The new `mc_public_channel_idx` and `mc_channels` fields on /health are additive; existing UI readers ignore them.
- Files modified:
  - `meshtastic-bridge/radio/meshcore_backend.py` — new `_public_channel_index` + `_channel_table` state; new async `_discover_channels()` (16-slot scan, sparse-table safe with ERROR→continue); wired into `_async_main` after `_refresh_contacts`; `send_broadcast` remaps channel=0 → resolved slot with logging; new `get_public_channel_index()` + `get_channel_table()` getters for the dashboard.
  - `meshtastic-bridge/standalone_bridge.py` — `_bridge_send` for MT: `wantAck=True` + `onResponse=self._on_bridge_ack_nak`, gated by new `_interface_ready()` helper; MT→MC path adds a pre-send log line with the resolved slot via new `_resolved_mc_channel()` helper; new `_on_bridge_ack_nak()` callback logs ACK/NAK per packet id.
  - `meshtastic-bridge/dashboard.py` — `/api/bridge/health` gains `mc_public_channel_idx` + `mc_channels`; `bridgeRhPollHealth()`'s backends-row renderer appends `· Public=ch<N>` or a fallback-warning when appropriate.
  - `meshtastic-bridge/tests/test_radio_backends.py` — new `TestMeshCoreChannelDiscovery` class with 6 tests: picks Public-named slot, case-insensitive match, no-Public defaults to 0, empty-table defaults to 0, send_broadcast remaps 0→resolved, send_broadcast preserves explicit non-zero.
  - `meshtastic-bridge/tests/test_bridge_selftest_endpoint.py` — 2 new `TestHealthEndpoint` tests: health surfaces `mc_public_channel_idx` + `mc_channels` when MC backend registered; both fields null/empty when no MC backend.
- Tests: 373/365 pytest green (added 8 new). Browser verified via `preview_*`: Relay Health panel renders `mt ✓, mc ✓ · Public=ch2` when the mocked health payload has a Public slot at index 2; renders the `⚠ no Public slot — using ch0 fallback (ch0:Personal, ch1:Admin)` warning when no slot matches.
...
- What changed:
  - **Deleted the `+ RADIO` universal-picker button** from the header (`#hdr-add-radio` at the end of `.lo-bar`). The `○ MT` and `◇ MC` pills are now the only radio-connect entry points. Both pills were already wired to `showMtManage()` / `showMcManage()` which open the shared `#add-radio-modal` pre-targeted to the right protocol — removing the third button is a pure UI simplification, no behavior changes for the surviving paths.
  - **Rewired the first-boot wizard from a single-shot pop to a chained MT → MC state machine.** Replaced the `_firstBootModalShown` boolean with `_wizardStage` (0 = show MT modal, 1 = show MC modal, 2 = done). `checkConnectionForModal()` now reads `App.state.backends` per-protocol instead of the rollup `connected` bool: stage 0 pops `showMtManage()` when MT isn't connected; stage 1 pops `showMcManage()` when MT is connected (or stage 0 was cancelled) and MC isn't connected; stage 2 calls `wizardComplete()`. Fast-forward branches let returning users who already have MT (or both) connected skip straight past the matching stage without a flash-pop. Stage is also advanced inside `hideAddRadioModal()` so cancelling the MT modal still chains into MC — a MeshCore-only user just closes the first modal and lands in the second, without needing to find the header pill.
  - **Removed the `wizardComplete()` side-effect from `arShowSuccess()`.** Previously a first successful connect flipped the setup-complete flag inside the success panel — which would have suppressed the MC modal under the new chained flow. Now stage advancement happens in one place (`hideAddRadioModal`), so the wizard transitions cleanly from MT-success → DONE click → MC modal pop on next poll tick.
  - **Unified the success-panel body copy.** Dropped the divergent MT/MC branches in `arShowSuccess()` that each referenced the now-defunct `+ RADIO` button. Single template: `"<Meshtastic|MeshCore> is online. Add a <other> radio from the <◇ MC|○ MT> header button to auto-bridge public-channel traffic between networks."` The protocol-specific branding (teal filled-circle glyph for MT vs purple filled-diamond for MC, title color, modal-open title + intro blurb) is preserved — operator confirmed those are intentional.
  - **Swept stale `+ RADIO` references** in comments across `dashboard.py` (HTML block comment, state-update, `showMtManage` / `showMcManage` docstrings, init comment) and in the four non-historical README spots (Protocol Scope Selector, Header Radio Pills, Connecting your radios, Top bar dashboard UI bullet). Left the v2.5 RELAY HEALTH narrative's `"+ RADIO" modal` reference untouched — that's describing past behavior.
- Why:
  - Operator feedback on hardware: "There's like three different ways to connect these radios... the Meshtastic one works great, the MeshCore one works great, but they display things differently. Can we remove the + RADIO button? And on boot, the + MT option pops up first, then once you connect that the + MC option pops up?" The universal picker had become redundant once the `○ MT` / `◇ MC` pills existed, and the first-boot wizard only popped once without guiding users through both radios — a dual-radio rig user had to hunt for the second entry point on their own. Chaining the wizard walks new users through the expected setup for the common dual-radio case while still letting MC-only users opt out with a single cancel.
- Impact on project goals:
  - UI simplification milestone for the v1.0-ship radio-connect story: ONE entry point per protocol, no ambiguous "which button do I press" moment. First-boot UX now matches the operator's mental model — "connect this one, then that one" — instead of "here's a picker, figure it out." Post-connect display is now structurally identical across MT and MC (same sentence skeleton, protocol name swap), which was the other half of the operator's complaint.
  - Scope discipline: no backend changes. `/api/connection/switch` (MT) and `/api/backends/add` (MC) endpoints are untouched — the operator confirmed both already work; only the front-end orchestration changed. No new modal, no new JS helpers; `_wizardStage` replaces `_firstBootModalShown` and the existing `showMtManage` / `showMcManage` are reused verbatim for the wizard stages.
- Files modified:
  - `meshtastic-bridge/dashboard.py` — HTML (delete `#hdr-add-radio`, rewrite block comment), JS (delete two `hdr-add-radio` styling branches, rename `_firstBootModalShown` → `_wizardStage`, rewrite `checkConnectionForModal`, extend `hideAddRadioModal` with stage-advance, delete `wizardComplete()` from `arShowSuccess`, unify success-panel description template, sweep stale comments).
  - `README.md` — four dual-radio narrative spots updated to describe the chained MT → MC wizard and the `○ MT` / `◇ MC` pills as the only connect entry points.
- Tests: 365/365 pytest green. Browser verification via `preview_*`: first-boot auto-pops MT modal (stage 0, protocol=meshtastic, `+ RADIO` button confirmed absent); cancelling MT advances to stage 1 and MC modal auto-pops (protocol=meshcore); cancelling MC calls `wizardComplete()` and sets the localStorage flag; reload leaves modal closed for returning users; manual pill clicks still work post-setup; success-panel copy reads identically for both protocols modulo the name/pill swap.
...
- What changed:
  - **`/api/bridge/health` now synthesizes a primary-MT backend entry.** v2.5 built its `backends` array solely from `_radio_manager.get_backends()`, but the primary Meshtastic radio lives outside RadioManager on the legacy `self.interface` path (spawned by `_radio_connection_loop` at startup); only the secondary (MC) is registered with RadioManager. The result was a silently-dishonest Relay Health panel: a dual-radio rig with both radios connected and relaying cleanly rendered as `mc ✓` alone with a yellow dot, because MT was invisible to the endpoint. v2.5.1 prepends a synthesized `{"protocol": "mt", "connected": bool(_bridge._is_interface_alive()), "primary": True}` entry to the response, and the RadioManager loop now tags its entries with `"primary": False`. Existing JS reader untouched — the additive `primary` field is ignored by it, and the existing "green if ≥2 connected, yellow if 1, red if 0" traffic-light logic now reflects reality.
  - **`_is_interface_alive()` catch broadened from `(AttributeError, OSError)` to `Exception`.** The probe's whole purpose is "touching `self.interface.localNode` raises if the interface is broken" — a `ConnectionResetError`, `TimeoutError`, or `meshtastic.MeshInterfaceError` from a half-disconnected USB radio is every bit as much a "not alive" signal as an `AttributeError`. The narrower catch let unexpected exception types propagate silently or made the check false-positive, so `_bridge_send` trusted the result and queued `sendText()` into a zombie interface. Same broadening applied to `MeshtasticBackend.is_connected()` at `radio/meshtastic_backend.py:132` for parity if a secondary-MT path is ever added.
  - **`_bridge_send` emits a red WARNING log before raising `ConnectionError` on a dead MT interface.** `relay.py` already records the exception as `send_error:ConnectionError:primary meshtastic interface not alive` in `_last_drop_reason_by_direction` (surfaces in Block 2's stats sub-line), but the dashboard log-tail viewer (Block 3) had nothing to show — the failure was silent in the live log. A single `logger.warning("[bridge] MT send refused — interface not alive (check serial/USB connection)")` call before the raise gives the operator a second visible signal, colour-coded red in the Block 3 viewer.
  - **Test helper realignment + 3 new tests in `TestHealthEndpoint`.** `_make_bridge_with_real_relay` in `tests/test_bridge_selftest_endpoint.py` previously mocked MT as a secondary backend via `get_backends()` — a topology that contradicted reality. The helper now sets `bridge.interface = MagicMock()` + `bridge._is_interface_alive = MagicMock(return_value=True)` and returns only the MC backend from `get_backends()`, matching production. Three new tests cover the three primary-MT states: `test_health_primary_mt_alive` (default helper → `connected=True`, `primary=True`), `test_health_primary_mt_dead` (override alive to False → `connected=False`), and `test_health_primary_mt_probe_exception_treated_as_dead` (override alive to `side_effect=RuntimeError` → endpoint still returns 200, `connected=False`, no exception propagates).
- Why:
  - Live hardware session: operator observed "MeshCore backend is connected but MT backend isn't" on the BRIDGE tab Relay Health panel AND reported that messages relayed MC→MT weren't reaching the other Meshtastic radio. Log excerpt from the run showed `[bridge] meshcore->meshtastic ch0 ...` lines firing cleanly with no `send failed` warnings — meaning the bridge's send path was returning normally. Two overlapping bugs could explain both symptoms simultaneously: (a) the panel was lying about MT because it read from the wrong data source, and (b) `sendText()` was silently queuing into a zombie interface because the liveness probe had too narrow a catch. The operator couldn't distinguish them from the UI because the UI wasn't showing MT state at all. v2.5.1 fixes (a) fully and tightens (b); an active heartbeat on the MT interface (synthetic traceroute / node-info poll) is the remaining gap but is a larger change deferred to v2.6+.
- Impact on project goals:
  - v2.5.1 is a point-release under the v2.5 integration-reliability banner — no new feature surface, just correcting v2.5's own blind spots. The Relay Health panel was the most significant operator-observability investment in the v1.0 ship, so a panel that silently omits a backend is worse than no panel (it actively misinforms). Backward-compat preserved: the `primary` boolean is additive, existing JS readers ignore unknown keys, and the endpoint response shape is otherwise identical.
  - Scope discipline held: no migration of primary MT into RadioManager (would be a structural refactor — `self.interface` is load-bearing for `_on_receive`, `_radio_connection_loop`, and many `sendText()` call sites), no new "submitted vs transmitted" distinction in `relay.py`, no active heartbeat. v2.5.1 is the minimum-diff release that restores truth-telling in the panel and stops silent drops on a zombie interface.
- Files modified:
  - `meshtastic-bridge/dashboard.py` — `api_bridge_health` prepends a synthesized primary-MT entry; all backend entries gain a `primary` boolean.
  - `meshtastic-bridge/standalone_bridge.py` — `_is_interface_alive` catch broadened to `Exception`; `_bridge_send` MT guard emits a `logger.warning` before raising.
  - `meshtastic-bridge/radio/meshtastic_backend.py` — `is_connected` catch broadened to `Exception` (parity with `_is_interface_alive`).
  - `meshtastic-bridge/tests/test_bridge_selftest_endpoint.py` — `_make_bridge_with_real_relay` helper realigned (primary MT on `bridge.interface`, only MC in `get_backends()`); 3 new `TestHealthEndpoint` tests.
- Tests: 365/365 pass (adds 3 new health-endpoint tests over the v2.5 baseline of 362).

---

## [2026-04-19 22:00] — v2.5: Cross-protocol gateway fix + Relay Health panel

- What changed:
  - **`Policy.should_relay()` refactored from `bool` → `(bool, reason: str)` tuple** across all four concrete policies (`DisabledPolicy`, `AlwaysRelay`, `ChannelAllowlist`, `AIGatedPolicy`). Reason strings form a stable diagnostic vocabulary the self-test endpoint and log tail surface: `policy:disabled`, `policy:dm_blocked`, `policy:no_rule_matched source=meshcore channel=3`, `policy:ai_not_urgent`, empty string on allow. The single call site in `relay.py` unpacks both halves; every `continue` in the guard chain now also writes to `_last_drop_reason_by_direction` with a matching reason (`empty_text`, `loop_guard:already_bridged`, `force_prefix_empty_body`, `dm_not_relayed`, policy-forwarded, `dedup`, `rate_limited:60s`, `send_error:ExceptionType:msg`).
  - **`Relay.observe()` gained a top-of-function INFO log** so operators can see "message reached the relay" distinctly from drop reasons. The log tags self-test traffic as `[SELFTEST]` so the live log tail can colorize it separately.
  - **`Relay.observe(..., is_selftest=True)` keyword-only arg.** Exercises the full guard chain (dedup, policy, rate-limit, loop-guard) but short-circuits `send_fn` — no radio traffic leaves the laptop. Tagged `is_selftest=True` on the `on_relay` event dict so the dashboard can filter self-test events out of the main messages tab while keeping them visible in the log tail. `_last_observe_at` (the wiring-alive indicator) is NOT updated by self-test runs — it reflects real traffic only. New `Relay.run_selftest(direction)` returns the endpoint's response dict: `{direction, ok, nonce, delivered, elapsed_ms, drop_reason}`.
  - **`Relay.stats()` surface grew** with `last_drop_reason_by_direction`, `rate_limited_by_direction`, `last_observe_at_s_ago`, `uptime_s`, and a per-direction `rate_limited` count inside the existing `by_direction` block.
  - **Startup auto-seed backstop** in `standalone_bridge.py`: `_auto_seed_when_both_radios_up` is a one-shot daemon thread spawned from `start()` that polls until two backends are connected, then — if `_bridge_config.rules` is empty — calls `_seed_default_bridge_rules(force=True)`. Fires at most once per process (via `_auto_seed_fired`), never overwrites custom rules, logs a clear INFO line on fire (`[bridge] auto-seeded default rules (mt ch0 always, mc ch0 always, enabled=true)`). The two existing seed call sites (`_spawn_secondary_radio:1466`, `add_secondary_radio:1534`) now set `_auto_seed_fired=True` too, so the backstop doesn't re-fire after an explicit user add.
  - **Three new dashboard endpoints.** `POST /api/bridge/selftest` (body `{"direction": "mt_to_mc" | "mc_to_mt" | "both"}`, returns `{ok, results: [{direction, ok, nonce, delivered, elapsed_ms, drop_reason}]}`). `GET /api/bridge/health` (compact `{master_enabled, rules, backends, last_observe_at_s_ago, ready}` snapshot for the Block 1 card). `GET /api/bridge/logs?limit=N` (filters the shared `DashboardLogHandler` ring buffer for `[bridge]` lines).
  - **RELAY HEALTH panel in the BRIDGE tab.** Four blocks: (1) live status card with four traffic-light dots and an inline master-switch toggle; (2) mirrored MT→MC / MC→MT stats table for Relayed / Dropped / Rate-limited plus a subline with dedup cache, rate-limit window, and uptime; (3) scrolling log-tail viewer with colour-coded lines (green = relay success, yellow = drops, red = send failures, cyan = `[SELFTEST]`), auto-scroll, copy-20-lines button; (4) "Run Relay Self-Test" button with per-direction result rendering. The `?` hover on Block 1's "Relay wiring" indicator explains the diagnostic logic in plain English.
- Why:
  - User report on hardware: cross-protocol DMs work, AI works, but MT↔MC public-channel relay is "90% there — just the final pieces where the protocols need to link together." The spec hypothesized three failure modes: (1) rules not seeded, (2) `observe()` missing in one path, (3) `enabled=False`. Exploration proved #2 wrong — `observe()` is already wired in both paths (`standalone_bridge.py:1081` for MT, `:1714` for MC). Real gaps were: seeding only fired on two of three entry paths (persisted-config restore was unseeded), and the relay had no operator-visible diagnostics — a dropped-for-no-matching-rule message was silent. v2.5 closes both gaps: the backstop guarantees seeded defaults regardless of entry path, and the Relay Health panel makes every drop self-explanatory so debugging becomes "read the reason string" instead of "stare at silent logs at 11pm." The `should_relay` tuple refactor is load-bearing for the diagnostics — without a reason string, self-test failures would just say "dropped" with no way to act on it.
- Impact on project goals:
  - v2.5 is the integration-reliability + observability release that seals MT↔MC public-channel relay for v1.0 ship. No rewrite of `bridge/relay.py` or the receive-path wiring — the core logic was already correct; v2.5 adds the instrumentation and startup safety net that make it reliable in the hands of an operator who didn't write the code. The self-test panel also enables future debugging to happen in seconds through the dashboard UI rather than requiring a terminal attached to the bridge.
  - Backward compatibility preserved: `observe()` gained a keyword-only kwarg so all existing callers keep working unchanged. `should_relay` tuple change is contained — the test helper in `tests/test_bridge_policy.py` was updated to unpack; all other legacy bool consumers were only internal to `relay.py`.
- Files modified:
  - `meshtastic-bridge/bridge/policy.py` — tuple return for all four policies with stable reason vocabulary.
  - `meshtastic-bridge/bridge/relay.py` — top-of-observe log, `is_selftest` kwarg, drop-reason tracking at every `continue`, `_last_observe_at` / `_started_at` / `_rate_limited_by_direction` / `_last_drop_reason_by_direction` state, `run_selftest()` method, expanded `stats()`.
  - `meshtastic-bridge/standalone_bridge.py` — `_auto_seed_when_both_radios_up()` method + wiring in `start()`, `_auto_seed_fired` flag set in both existing seed paths.
  - `meshtastic-bridge/dashboard.py` — three new endpoints (`/api/bridge/selftest`, `/api/bridge/logs`, `/api/bridge/health`); Relay Health panel HTML + CSS + JS added to the BRIDGE tab (Blocks 1–4); `bridgeActivate` now also polls health / logs / stats on the 2.5s tick.
  - `meshtastic-bridge/tests/test_bridge_policy.py` — `_call` helper updated to unpack the tuple so existing bool assertions keep working.
  - `meshtastic-bridge/tests/test_bridge_relay.py` — `test_stats_by_direction_always_present` updated for the new `rate_limited` field; 16 new tests for `is_selftest`, drop-reason tracking, `run_selftest`, and new stats keys.
  - NEW `meshtastic-bridge/tests/test_bridge_policy_reasons.py` — 13 tests covering the drop-reason vocabulary for every policy branch.
  - NEW `meshtastic-bridge/tests/test_bridge_selftest_endpoint.py` — 14 integration tests exercising `/api/bridge/selftest`, `/api/bridge/health`, `/api/bridge/logs` against a real `Relay` instance + mocked radio manager.
- Tests: 362/362 pass (adds 47 new tests: 16 relay, 13 policy-reasons, 14 endpoint).
...
- What changed:
  - **Legacy Meshtastic backend now always surfaces in `/api/state` alongside any RadioManager-registered MC backend.** Root cause of the "MT shows OFF as soon as MC attaches" bug: MT runs through the legacy `_radio_connection_loop` and is never registered with `RadioManager`, so `get_backends_info()` returned only `[MC]` once a secondary radio came up, and the MT status pill went dark. `api_state()` now injects a synthetic MT backend entry (with `self_node_id` derived from `_get_self_node_id`) whenever RadioManager's list lacks an MT-protocol entry — both pills stay accurate in every combination.
  - **MC MY NODE now actually renders as a purple diamond.** `buildGraph()`'s early-exit was fingerprinting only `known_nodes` + `positions` + channels — not `state.backends` — so when MC attached mid-session the `selfBackends.forEach()` path never re-ran and the MC self-node never made it into `App.nodes`. Fixed by appending a backend fingerprint (`id:connected:self_node_id` joined with `|`) to `currentIds`, so a new backend forces the rebuild. The diamond + purple colour path was already in the draw loop and just needed the rebuild to fire.
  - **`mc:channel:0` is now registered as a node whenever MC is connected**, same treatment the `meshtastic:channel:0` pseudo-node has had since day one. When both public channels exist on-canvas at the same time their labels become `PUBLIC MT` / `PUBLIC MC` so two indistinguishable "PUBLIC" rings don't overlap in `ALL` scope.
  - **Two pinned PUBLIC CHANNEL rows at the top of the NODES sidebar.** New `#ns-pinned` block above the sort header, populated by `renderPinnedChannels()` on every `renderNodeList()` tick and every poll. Rows visible when the matching backend is connected; click routes through `openFloatWindow()` → `/api/threads/.../send` (which already handles channel-0 routing to the right backend). Icons match the protocol: teal circle for MT, purple diamond for MC. Pinned entries are filtered out of the scrolling list below so the same target doesn't appear twice.
  - **Header `◆ MT` pill mirroring the existing `◆ MC` pill.** New `#hdr-manage-mt` button between the MT status text and the MC pill. Reads `◇ MT` when idle, `◆ MT` when connected; tint flips teal via `.lo-conn-add-mt.on`. Click opens the MT self-window (battery / voltage / hw / uptime) — same panel the canvas MY NODE click has always opened. Falls through to the connect modal when MT isn't paired yet.
  - **MC-flavoured HUD rail for `mc` scope.** In `ALL` / `MT` scope the HUD still shows `SCAN MESH` + `HW COLOR` + the hardware-colour legend. Switch to `MC` and the rail swaps to `PULL CONTACTS` (new `POST /api/nodes/refresh_mc` that kicks `_refresh_contacts()` on every MC backend and then runs `_sync_nodes_from_backends()`), `FAVORITES` (toggles `App.mcFavoritesOnly`, plumbed through `nodeInScope()` so both canvas and sidebar honour it), a MeshCore advertisement legend, and live counts (`N contacts · N with GPS · N starred`). Empty-state hint still appears when MC is up but no contacts have been discovered yet.
  - **Bridge smoke-test button in the BRIDGE tab.** `POST /api/bridge/smoke_test` now does two things: (1) real broadcast on channel 0 from every connected radio so nearby peers hear the tagged ping, then (2) synthetic `_relay.observe()` calls with a unique-per-run fake sender in each direction, which actually exercises the relay (broadcasting from our own radios never triggers it — the relay only observes *incoming* peer traffic). Response includes `stats_before` + `stats_after` so the UI can show the inline delta without a 6 s polling loop. JS uses the correct `meshtastic->meshcore` / `meshcore->meshtastic` stat keys (previous iteration watched the UI-shortened `mt->mc` keys which never existed on the server).
  - **Scope-selector active button now closes its right border.** The segmented control used `border-right: none` on all but the last button, so an active middle button (`MT`) read as a three-sided U. Active state now restores the right border in the accent colour with `z-index: 1` so the edge doesn't get clipped by the neighbour's left border.
  - **`/api/connection/switch` stopped 500ing on `address: null`.** Call was `data.get("address", "").strip()` which blew up with `AttributeError: 'NoneType' object has no attribute 'strip'` when the JSON body passed an explicit `null`. Fixed by using `(data.get("address") or "").strip()` on the three affected fields (`type`, `address`, `host`).
  - **Node-sync log line** in `_sync_nodes_from_backends` that reports `merged N new MeshCore contact(s)` whenever MC contacts arrive — silent diagnostic when a user reports MC nodes aren't showing.
- Why:
  - After the earlier equal-citizens rollout the scope selector + per-direction bridge stats + MC scope filter were all in place, but the *operator experience* of attaching a MeshCore radio was still janky: status pill showed MT as disconnected, MC MY NODE never appeared, the HUD rail was full of Meshtastic-only buttons, talking on the 915 MHz public channel meant scrolling for a channel node that may or may not exist, and there was no way to prove the bridge was actually wired through without pulling out a second device. This pass closes every one of those gaps. The smoke test in particular is the piece I kept needing during debugging — a one-click "is the relay actually relaying" that gives a definitive pass/fail in ≤1 s.
- Impact on project goals:
  - Full symmetry between the two protocols in the dashboard chrome: every surface that the MT radio has (status pill + management pill + SCAN MESH + colour legend + MY NODE + self-settings panel + pinned public-channel row) the MC radio now has too, in matching purple-diamond style. The smoke test gives operators a standing validation they can run any time (boot, bridge-config change, suspected outage) without hardware ceremony. The `api_state` fix removes what was effectively a regression from the multi-backend refactor — any rig that attached MC via the add-radio flow has been showing misleading MT status for weeks. Bridge-stats JS key alignment also means the counter numbers in the UI now actually reflect live server data instead of perpetually reading zero.
- Files modified:
  - `meshtastic-bridge/dashboard.py` — `api_state` MT-backend injection; scope-button border fix; `/api/connection/switch` None-safe strip; new `/api/bridge/smoke_test`; new `/api/nodes/refresh_mc`; HUD MC-controls block + `.lo-mc-btn` styles; MC empty-state hint; `◆ MT` pill + `.lo-conn-add-mt.on`; pinned public-channel rows + `.lo-ns-pinned-row` styles; `openPublicChannel`, `showMtManage`, `refreshMcContacts`, `toggleMcFavoritesOnly`, `updateHudScopeControls`, `renderPinnedChannels` JS; backend-aware `isMC` channel-0 node, label disambiguation; `nodeInScope` honours `App.mcFavoritesOnly`; `buildGraph` guard now fingerprints backends.
  - `meshtastic-bridge/standalone_bridge.py` — `_sync_nodes_from_backends` logs merged MC contact counts.
  - `README.md` — Node List Sidebar + HUD Overlay sections rewritten to cover pinned PUBLIC CHANNEL rows and the scope-swapped MC rail; new sections for the `◆ MT` + `◆ MC` pills and the BRIDGE tab smoke test.
- Tests: 315/315 pass.
