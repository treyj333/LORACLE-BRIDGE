...
### Meshtastic Mesh Network Bridge
N.O.M.A.D. includes an optional **Meshtastic Bridge** that connects a Meshtastic LoRa radio to the local AI assistant. This allows users on the mesh network to send text questions over radio and receive AI-generated responses — no internet or WiFi required.

**How it works:**
- A Python sidecar service connects to a Meshtastic device via USB serial or TCP
- Incoming mesh messages are forwarded to N.O.M.A.D.'s LLM API
- LLM responses are chunked to fit within LoRa's 228-byte message limit and sent back over the mesh
- Optional RAG (Knowledge Base) support for mesh queries
- Configure via Settings > Meshtastic in the Command Center

**Mesh commands:** Users on the mesh can prefix messages with `!rag`, `!norag`, `!status`, `!help`, or `!model <name>` to control behavior.

**Setup:** Install the Meshtastic Bridge from Settings > Apps, then configure the connection type (Serial/TCP) in Settings > Meshtastic.
