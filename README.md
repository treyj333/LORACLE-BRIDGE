...
The first-run wizard is a chained **MT → MC** flow: on first boot the Meshtastic connect modal auto-pops; once you've connected MT (or cancelled the modal) the MeshCore modal auto-pops next. Cancelling either step still advances the chain, so a MeshCore-only user just closes the MT modal and lands in the MC one without needing a separate entry point. Setup is marked complete once you cancel or connect through the MC step; the `○ MT` / `◇ MC` header pills remain available afterwards for manual connect or switch.
...
Right of the two connection indicators (`MT ON` / `MC ON`) sit a pair of **management pills**, one per protocol. Shape + colour match the protocol: **teal circle (`● MT`)** for Meshtastic, **purple diamond (`◆ MC`)** for MeshCore. Each pill always opens the radio-connect modal pre-targeted to that protocol — disconnected users get a connect form, already-connected users can switch transport or swap devices. When a radio isn't connected the pill shows its outline glyph (`○ MT`, `◇ MC`); when connected it fills in (`● MT`, `◆ MC`).
...
**Connecting your radios — zero CLI required.** The first time you open the dashboard, a chained MT → MC wizard walks you through connecting a Meshtastic radio (step 1) and a MeshCore radio (step 2). Either step can be skipped — cancelling the MT modal still advances to MC — so a MeshCore-only user just closes the first modal and lands in the MC one.

Each step has a **SCAN PORTS** button (serial) and a **SCAN BLE** button (Bluetooth):
- **Serial scan** lists every available device on your system with a green-dot hint on the ones that look like a LoRa radio (CP210x / CH340 / FT232 / manufacturer-name match).
- **BLE scan** is protocol-aware. For Meshtastic, it uses the Meshtastic service UUID so the list is pre-filtered to MT radios. For MeshCore, it scans at the bleak layer and tags devices that advertise the **Nordic UART Service UUID** (`6E400001-…`) or whose name starts with `MeshCore-` / `mc-` — these are flagged as `◆ likely MeshCore` and shown first. By default the MC BLE list hides non-MC devices (no more picking a MeshCore radio out of a crowd of TVs, earbuds, and unknowns); toggle **"Show all BLE devices"** to fall back to the unfiltered list if the heuristic misses your radio.

Click a row to fill in the address, then CONNECT. After each successful connect you get a confirmation screen (`● MESHTASTIC CONNECTED` / `◆ MESHCORE CONNECTED`) with a DONE button so the flow is user-paced, not a flash-then-close. After setup, the `○ MT` / `◇ MC` header pills open the same connect modal any time for a re-pair or transport switch. The spec is persisted to `~/.mesh-llm/settings.json` so the bridge auto-restores it on the next restart — the `--second-radio` CLI flag still works and wins if both are set.
...
- **Top bar** shows each backend's connection state independently — a teal circle for Meshtastic (`● MT ON/OFF/--`) and a purple diamond for MeshCore (`◆ MC ON/OFF/--`). Right of those sit matching connect/manage pills: `○ MT` / `● MT` (circle) opens the Meshtastic connect modal, `◇ MC` / `◆ MC` (diamond) opens the MeshCore connect modal. Both pills are always present regardless of connection state — no separate add-radio button.
